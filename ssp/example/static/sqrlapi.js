...
// Based on the example file here https://sqrl.grc.com/sqrlapi/sqrlapi.js     //
//============================================================================//

window.SQRL = window.SQRL || {}

window.SQRL.inject = function(config) {
    ...
    var sqrlEp = sqrlBaseUrl.indexOf('http') > -1 ? 
        ...
        // TODO: Clean this up - will break if config.api does not start with /
        'sqrl://' + window.location.host + sqrlBaseUrl + '/cli.sqrl';
    var qrEp = sqrlBaseUrl + '/qr.png'
    ...
    //============================================================================//
    function getSqrlNut() {
        syncQuery2.open('GET', nutEp);	// the page's DOM is loaded
        syncQuery2.onreadystatechange = function () {
            if (syncQuery2.readyState === 4) {
                if (syncQuery2.status === 200) {
                    ...
                    setTimeout(getSqrlNut, 100); // if our request for a nut fails, wait 10msec and retry
                }
            }
        };
    ...
    function pollForNextPage() {
        ...
        syncQuery1.open('GET', sqrlBaseUrl + '/pag.sqrl?nut=' + sqrlNut);	// the page is visible, so let's check for any update
    ...
    gifProbe.onload = function () {  // define our load-success function
        ...
        var encodedSqrlUrl = window.btoa(sqrlBaseUrl).replace(/\//, "_").replace(/\+/, "-").replace(/=+$/, "");
